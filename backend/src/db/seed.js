...
async function seed() {
  try {
    ...
    const existingSettings = await db.query(
      'SELECT id FROM automation_settings WHERE workshop_id = $1',
      [workshopId],
    );
    ...
    for (const [name, mobile, email] of customers) {
      let result = await db.query(
        'SELECT id FROM customers WHERE workshop_id = $1 AND mobile = $2',
        [workshopId, mobile],
      );
      if (!result.rows[0]) {
    ...
    for (const v of vehicles) {
      let result = await db.query(
        'SELECT id FROM vehicles WHERE workshop_id = $1 AND registration_number = $2',
        [workshopId, v[1]],
      );
      if (!result.rows[0]) {
        result = await db.query(
    ...
    const advisorResult = await db.query('SELECT id FROM users WHERE username = $1', ['advisor']);
    ...
    const appointmentCount = await db.query(
      'SELECT COUNT(*) AS count FROM appointments WHERE workshop_id = $1',
      [workshopId],
    );
    ...
    const templateCount = await db.query(
      'SELECT COUNT(*) AS count FROM inspection_templates WHERE workshop_id = $1',
      [workshopId],
    );
    ...
    const inventoryCount = await db.query(
      'SELECT COUNT(*) AS count FROM inventory WHERE workshop_id = $1',
      [workshopId],
    );
    ...
    const roExisting = await db.query(
      'SELECT TOP 1 id FROM repair_orders WHERE workshop_id = $1 ORDER BY created_at DESC',
      [workshopId],
    );
    ...
    if (!roExisting.rows[0]) {
      ...
      await db.query(
        `INSERT INTO complaints (repair_order_id, description, source)
         VALUES ($1, $2, $3)`,
        [roId, 'Brake squeaking noise while driving', 'manual'],
      );
      await db.query(
        `INSERT INTO complaints (repair_order_id, description, source)
         VALUES ($1, $2, $3)`,
        [roId, 'AC not cooling properly', 'manual'],
      );
